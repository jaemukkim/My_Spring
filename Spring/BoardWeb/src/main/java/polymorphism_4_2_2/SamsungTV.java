...
public class SamsungTV implements TV {
	...
	private SonySpeaker speaker;	//Speaker 는 인터페이스 : 
	private int price; 		// 정수 변수 선언
	
	public SamsungTV() {  //기본생성자 : 객체가 생성될때 호출
	...
	public SamsungTV(SonySpeaker speaker) {
		System.out.println("===> SamsungTV 객체 생성2 : 생성자 객체 주입");
		this.speaker = speaker;
	}
	
	//생성자 : 매개 변수가 2개인 생성자
	public SamsungTV(SonySpeaker speaker, int price) {
		System.out.println("===> SamsungTV 객체 생성3 : 매개변수 2개인 생성자 호출");
		this.speaker = speaker;
		this.price = price;
	}
	...
	//감결합 상태, 프로그램 내에서 객체를 생성할 경우, 새로운 기능의 스피커로 교체할 때 코드를 수정해야 한다.
