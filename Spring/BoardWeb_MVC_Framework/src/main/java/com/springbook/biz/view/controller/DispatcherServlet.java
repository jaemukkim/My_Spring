...
public class DispatcherServlet extends HttpServlet {
	...
	//HttpServlet 클래스의 init() 메소드 재정의 : 톰켓서버가 실행될때 init 메소드가 호출됨
		//init() 메소드는 초기값 설정을 하는 메소드
	
	@Override
	public void init() throws ServletException {
		System.out.println("톰켓 서비스 시작시 호출 - init()");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//클라이언트에서 Post 방시으로 들어오는 모든 요청을 process 메소드에서 처리하도록 던져줌
	...
	private void process (HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//3. 검색된 Controller를 실행한다. 리턴 값으로 뷰페이지를 리턴 받아온다.
